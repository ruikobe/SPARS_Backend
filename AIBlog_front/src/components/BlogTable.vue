...
<template>
  <div>
    ...
    <el-table
      ...
      <el-table-column
        label="Recent modification time" width="140" align="left">
      ...
      <el-table-column label="Operation" align="left" v-if="showEdit || showDelete">
        <template slot-scope="scope">
          ...
          <el-button
            size="mini"
            @click="handleRestore(scope.$index, scope.row)" v-if="showRestore">Restore
          </el-button>
          <el-button
            size="mini"
            type="danger"
            @click="handleDelete(scope.$index, scope.row)" v-if="showDelete">Delete
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    <div class="blog_table_footer">
      <el-button type="danger" size="mini" style="margin: 0px;" v-show="this.articles.length>0 && showDelete"
                 :disabled="this.selItems.length==0" @click="deleteMany">Delete in Batches
      </el-button>
      <span></span>
      <el-pagination
        background
  ...
  export default{
    ...
    methods: {
      ...
      handleRestore(index, row) {
        let _this = this;
        this.$confirm('Confirm to restore？','Warning',{
          ...
          putRequest('/article/restore', {articleId: row.id}).then(resp=> {
            if (resp.status == 200) {
              ...
              _this.$message({type: 'error', message: 'Restoration Failed!'});
            }
            _this.loading = false;
          });
        }).catch(() => {
      ...
      deleteToDustBin(state){
        var _this = this;
        this.$confirm(state != 2 ? 'Move to Trash?' : 'Confirm to permanently delete?', 'Warning', {
          confirmButtonText: 'Confirm',
          cancelButtonText: 'Cancel',
          type: 'warning'
        }).then(() => {
          ...
          putRequest(url, {aids: _this.dustbinData, state: state}).then(resp=> {
            if (resp.status == 200) {
              ...
              _this.$message({type: 'error', message: 'Delete Failed!'});
            }
            _this.loading = false;
            _this.dustbinData = []
          }, resp=> {
