    ...
    <div style="display: flex;justify-content: space-around;flex-wrap: wrap">
      <el-card style="width:330px;margin-top: 10px;" v-for="(user,index) in users" :key="index"
        ...
        <div>
          ...
          <div style="text-align: left;color:#20a0ff;font-size: 12px;margin-top: 13px">
            <span>Registration Time:</span><span>{{user.regTime | formatDateTime}}</span>
          </div>
          <div
            ...
            <el-switch
              ...
              inactive-text="Deactive" style="font-size: 12px">
            </el-switch>
          </div>
          <div style="text-align: left;color:#20a0ff;font-size: 12px;margin-top: 13px">
            <span>User Role:</span>
            ...
            <el-popover
              ...
              <el-select v-model="roles" :key="user.id" multiple placeholder="Please select" size="mini">
                <el-option
    ...
    methods: {
      saveRoles(id, index){
        ...
        putRequest("/admin/user/role", {rids: this.roles, id: id}).then(resp=> {
          if (resp.status == 200 && resp.data.status == 'success') {
            ...
            _this.$message({type: 'error', message: 'Update Failed!'});
          }
        }, resp=> {
          _this.cardloading.splice(index, 1, false)
      ...
      deleteUser(id){
        ...
        this.$confirm('Confirm to DELETE this user?', 'Warning', {
          ...
          deleteRequest("/admin/user/" + id).then(resp=> {
            if (resp.status == 200 && resp.data.status == 'success') {
              _this.$message({type: 'success', message: 'Delete Successfully!'})
              _this.loadUserList();
              return;
            }
            _this.loading = false;
      ...
      enabledChange(enabled, id, index){
        ...
        putRequest("/admin/user/enabled", {enabled: enabled, uid: id}).then(resp=> {
          ...
          _this.cardloading.splice(index, 1, false)
          _this.$message({type: 'success', message: 'Update Successfully!'})
        }, resp=> {
          _this.$message({type: 'error', message: 'Update Failed!'})
          _this.loadOneUserById(id, index);
        });
